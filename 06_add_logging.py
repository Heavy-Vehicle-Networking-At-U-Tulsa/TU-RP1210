...
class RP1210ReadMessageThread(threading.Thread):
    ...
    def run(self):
        ...
        with open("ReceiveNetworkTraffic.log",'w') as log_file:
            while self.runSignal:
                ...
                if return_value > 0:
                    ...
                    log_file.write("{:12.6f} ".format(time.time()) + " ".join("{:02X}".format(b) for b in ucTxRxBuffer[:return_value]) + '\n')
                time.sleep(.0005)
...
class RP1210Class():
    ...
    def __init__(self,dll_name,protocol,deviceID,speed):        
        ...
        if dll_name is not None and protocol is not None and deviceID is not None:
            logger.debug("Loading the {} file using the {} protocol for device {:d}".format(dll_name + ".dll", protocol, deviceID))
            try:
            ...
            logger.debug("Connecting to ClientConnect using " + repr(protocol_bytes))
            try:
...
class TUDiagnostics(QMainWindow):
    ...
    def selectRP1210(self,automatic=False):
        ...
        try:
            ...
            if nClientID is not None:
                ...
                logger.debug("Exiting. RP1210_ClientDisconnect returns {}: {}".format(return_value,RP1210Errors[return_value]))
        ...
        if nClientID < 128 and nClientID is not None: 
            ...
            if return_value == 0:
                logger.debug("RP1210_Set_All_Filters_States_to_Pass - SUCCESS")
            else :
                logger.debug('RP1210_Set_All_Filters_States_to_Pass returns {:d}: {}'.format(return_value,RP1210Errors[return_value]))
                return
            ...
            logger.debug("There was an error. Client ID is {}".format(nClientID))
    ...
    def get_j1939_vin(self):
        ...
        if nClientID is not None or nClientID < 128:
            pgn = 65260
            logger.debug("PGN: {:X}".format(pgn))
            b0 = pgn & 0xff
            logger.debug("b0 = {:02X}".format(b0))
            b1 = (pgn & 0xff00) >> 8
            logger.debug("b1 = {:02X}".format(b1))
            dlc = 3
            ...
            if return_value != 0:
                ...
                if return_value in RP1210Errors:
                    logger.debug("RP1210_SendMessage fails with a return value of  {}: {}".format(return_value,RP1210Errors[return_value]))
                    ...
                    logger.debug("return value: {}: {}".format(return_value,RP1210Errors[return_value]))
                message_window.exec_()

        else:
            logger.debug("RP1210 Device Needs to be Connected")
            message_window = QMessageBox()
    ...
    def disconnectRP1210(self):
        nClientID = self.RP1210.nClientID
        logger.debug("Sending read_message_thread run signal a false.")
        ...
        if nClientID is not None:
            ...
            logger.debug("nClientID is " + repr(nClientID))
